...
class LineNode extends ContainerNode<LeafNode>
  ...
  LineNode splitAt(int index) {
    ...
    line.addFirst((child.cutAt(split.offset) as LeafNode));
    return line;
  }
  ...
  NotusStyle collectStyle(int offset, int length) {
    ...
    final data = lookup(offset, inclusive: true); //todo trace all lookup calls
    if (data.node != null) {
...
//          'It is not allowed to apply inline attributes to line itself.');
  ...
  void delete(int index, int length) {
    ...
    if (isLFDeleted && isNotEmpty) {
      ...
      moveChildren((nextLine as ContainerNode));
    }
