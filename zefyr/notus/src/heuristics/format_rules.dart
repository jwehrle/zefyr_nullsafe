...
import 'package:birch/zefyr/quill_delta.dart';
  ...
  Delta? apply(
    ...
    while (current < length && iter.hasNext) {
      ...
      if (lf != -1) {
        ...
        if (pos < op.length!)
          result.retain(op.length! - pos, attribute.toJson());
      } else {
        result.retain(op.length!, attribute.toJson());
      }
      current += (op.length as int);
    }
...
class FormatLinkAtCaretPositionRule extends FormatRule {
  ...
  Delta? apply(
    ...
    if (before != null && before.hasAttribute(attribute.key)) {
      startIndex -= (before.length as int);
      retain = (before.length as int);
    }
    if (after.hasAttribute(attribute.key)) {
      retain += (after.length as int);
    }
