...
library zefyr.quill_delta;

import 'dart:math' as math;

import 'package:collection/collection.dart';
import 'package:birch/zefyr/quiver_hashcode.dart';
...
typedef DataDecoder = String Function(String data);

/// Default data decoder which simply passes through the original value.
String _passThroughDataDecoder(String data) => data;
...
class Operation {
  ...
  factory Operation.insert(dynamic data, [Map<String, dynamic>? attributes]) =>
      Operation._(Operation.insertKey, data.length, data, attributes);
...
class Delta {
  ...
  static Map<String, dynamic>? invertAttributes(
    ...
    Map baseInverted = base.keys.fold({}, (memo, key) {
      if (base![key] != attr![key] && attr.containsKey(key)) {
        (memo as Map<String, dynamic>)[key] = base[key];
      }
      return memo;
    });
  ...
  void _mergeWithTail(Operation operation) {
    ...
    final lastText = last.data;
    final opText = operation.data;
  ...
  Operation? _transformOperation(
    ...
    final int length = math.min(
        (thisIter.peekLength() as int), (otherIter.peekLength() as int));
    final thisOp = thisIter.next(length);
    final otherOp = otherIter.next(length);
    assert(thisOp.length == otherOp.length);
  ...
  Operation next([num length = double.infinity]) {
    ...
    if (_index < delta!.length) {
      ...
      final opData = op.isInsert
          ...
          : op.data;
      final opIsNotEmpty = opData!.isNotEmpty; // embeds are never empty
      final opLength = opData.length;
