...
import 'package:birch/zefyr/notus/notus.dart';
import 'package:birch/zefyr/util.dart';
...
class ZefyrSelectionOverlay extends StatefulWidget {
  const ZefyrSelectionOverlay({required this.controls});
...
class ZefyrSelectionOverlayState extends State<ZefyrSelectionOverlay>
  ...
  bool get shouldHideControls {
    ...
    final isSelectionCollapsed = selection.isCollapsed;
  ...
  void _handleTap() {
    ...
    RenderEditableBox? editableBox = _getEditableBox(result);
    // RenderEditableProxyBox? box =
    //     editableBox != null ? (editableBox as RenderEditableProxyBox) : null;
    ...
    RenderEditableProxyBox? box = (editableBox != null)
        ? (editableBox as RenderEditableProxyBox)
        : _scope!.renderContext.closestBoxForGlobalPoint(globalPoint);
    if (box == null) return null;
...
class _SelectionHandleDriverState extends State<SelectionHandleDriver>
  ...
  Offset _getLocalPointFromDragDetails(DragUpdateDetails details) {
    ...
    _dragPosition!.translate(details.delta.dx, details.delta.dy);
    RenderEditableBox paragraph = (_scope!.renderContext
        .boxForGlobalPoint(_dragPosition!) as RenderEditableBox);
