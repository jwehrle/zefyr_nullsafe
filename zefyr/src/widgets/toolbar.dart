...
class ZefyrButtonList extends StatefulWidget {
  const ZefyrButtonList({required this.buttons});
...
class _DefaultZefyrToolbarDelegate implements ZefyrToolbarDelegate {
  ...
  Widget buildButton(BuildContext context, ZefyrToolbarAction action,
    ...
    VoidCallback buttonPressed =
        onPressed != null ? onPressed : _defaultCallback;
    if (kDefaultButtonIcons.containsKey(action)) {
      IconData? icon = kDefaultButtonIcons[action];
      //icon ??= Icons.all_inclusive_outlined;
      double? size = kSpecialIconSizes[action];
      //size ??= 16.0;
